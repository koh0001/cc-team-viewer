...
- **에이전트 상태** — 각 에이전트가 현재 무슨 작업 중인지, 모델(opus/sonnet/haiku), 백엔드 타입. 클릭/Enter로 담당 태스크 목록 확장
- **태스크 보드** — 표/칸반 보드 전환(`K`), 상태, 담당자, 의존성, 차단 관계. 클릭/Enter로 태스크 상세 패널 확장
- **메시지 로그** — 에이전트 간 통신 내역 실시간 표시, 필터(전체/대화/시스템, `F`로 순환) 및 스레드 그룹핑
- **의존성 그래프** — 위상 정렬 레이어 시각화, 박스 드로잉 노드(TUI) 및 CSS Grid + SVG Bezier 곡선(VS Code)
- **진행률 통계** — 완료율, 경과 시간, 에이전트별 처리량
- **키보드 내비게이션 (TUI)** — `Enter`/`Esc` 포커스 모드, `↑/↓` 커서, `K` 칸반 토글, `F` 메시지 필터
