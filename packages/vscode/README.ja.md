...
| タブ | 説明 |
|------|------|
| **Overview** | エージェントカード — ステータス、実行中タスク、進捗率。クリックで担当タスク一覧を展開 |
| **Tasks** | タスクテーブル/カンバンボード — クリックでインライン詳細パネル（説明、blockedBy/blocksリンク） |
| **Messages** | メッセージログ — フィルターボタン（全体/会話/システム）+ スレッドグルーピング |
| **Deps** | DAGグラフ — CSS Gridレイアウト + SVG Bezier曲線エッジで依存関係を可視化 |
...
### その他の機能
- **タスク詳細パネル** — タスクをクリックするとインラインで説明、ステータス、担当者、依存リンクを表示
- **メッセージスレッド** — 同一送信→受信の連続メッセージを折りたたみ可能なスレッドにグルーピング
- **リアルタイム通知** — タスク完了、エージェント参加/離脱時にVS Code通知ポップアップ
- **チームpill切り替え** — 複数チーム監視時のクイック切り替え
- **エージェントパルスアニメーション** — 作業中エージェントの視覚的表示
- **レンダー最適化** — アクティブタブのみレンダリング、データ未変更時は再レンダリングをスキップ
