...
- **エージェントステータス** — 各エージェントの作業内容、モデル(opus/sonnet/haiku)、バックエンドタイプ。クリック/Enterで担当タスク一覧を展開
- **タスクボード** — テーブル/カンバンボード切替(`K`)、ステータス、担当者、依存関係、ブロッキング。クリック/Enterでタスク詳細パネルを展開
- **メッセージログ** — エージェント間通信のリアルタイム表示、フィルター(全体/会話/システム、`F`で切替)とスレッドグルーピング
- **依存関係グラフ** — トポロジカルソートレイヤー可視化、ボックス描画ノード(TUI)およびCSS Grid + SVG Bezier曲線(VS Code)
- **進捗統計** — 完了率、経過時間、エージェント別スループット
- **キーボードナビゲーション (TUI)** — `Enter`/`Esc`フォーカスモード、`↑/↓`カーソル、`K`カンバン切替、`F`メッセージフィルター
